...
import { ChevronDown, ChevronLeft, ChevronRight, Home, Package, UserCircle, X } from "lucide-react";
...
const SubItem = ({ label, isActive, onClick }) => (
 ...
 const UserSidebarLinks = [
  { label: "Pibo Dashboard", icon: Home, path: "/pibo-dashboard" },
  // {
  //   label: "Registration",
  //   icon: UserCircle,
  //   expandable: true,
  ...
  //   ],
  // },
  // { label: "Packaging Declaration", icon: Package, path: "/user/packaging-declaration" },

];


const UserSidebar = React.memo(({ sidebarOpen, toggleSidebar, sidebarCollapsed = false, toggleSidebarCollapsed }) => {
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between gap-2 p-4 border-b border-gray-200 min-w-[16rem] xl:min-w-[18rem]">
          <img src="" alt="CLIMETO Logo" className="h-10 w-auto object-contain flex-shrink-0" />
        ...
        <nav className="flex-1 p-4 overflow-y-auto min-w-[16rem] xl:min-w-[18rem]">{UserSidebarLinks.map(renderMenu)}</nav>
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-gray-200">
          <img src="" alt="CLIMETO Logo" className="h-10 w-auto object-contain" />
          <button
        ...
        <nav className="flex-1 p-4 overflow-y-auto">{UserSidebarLinks.map(renderMenu)}</nav>
      </aside>
    </>
  );
});
